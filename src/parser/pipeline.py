...
def build(manifest, cache, output, log=lambda _: None):
    ...
    for record in sorted(manifest["documents"], key=lambda r: (r["kind"], r["source_url"])):
        ...
        if record["kind"] == "chapter":
            ...
            parsed = parse_source(data, number, year, record.get("columns", 2), cache.root / 'extracted')
            ...
            for ordinal, text in enumerate(parsed['chapterNotes']):
                tables['chapter_notes'].append(dict(id=digest(f'{year}:{number}:{ordinal}'.encode()),
                    edition_year=year, chapter_number=number, note_text=text))
            versions = {}
            note_ordinals = {}
            for section in parsed["sections"]:
                section_number = section["sectionNumber"]
                version = versions.get(section_number, 0)
                versions[section_number] = version + 1
                if version == 0:
                ...
                tables['section_versions'].append(dict(edition_year=year, ors_section=section_number,
                    version_ordinal=version, catchline=section['catchline'], content_text=section['bodyText'],
                    status=section['status'], source_credit=section['sourceCreditRaw'],
                    publication_notes=json.dumps([n['text'] for n in section['notes']], ensure_ascii=False)))
                ...
                for kind, text in dict.fromkeys(notes):
                    note_key = (section_number, kind, text)
                    if note_key in note_ordinals:
                        continue
                    ordinal = len(note_ordinals)
                    note_ordinals[note_key] = ordinal
            ...
            if data.startswith(b"%PDF"):
                document = pdf_document(data, columns=record.get("columns", 2), cache_dir=cache.root / 'extracted')
            else:
    ...
    notes = {"scope": manifest.get("scope", "manifest"), "notes": manifest.get("notes"),
             "supplements": [json.loads(s) for s in sorted({json.dumps(s, sort_keys=True) for s in supplements})]}
    ...
    return {"edition_year": year, "scope": manifest.get("scope", "manifest"),
            ...
            "archive_count_verified": manifest.get('expected_archive_count') == len(tables['chapters']),
            "alternate_versions": len(tables['section_versions']) - len(tables['sections']),
