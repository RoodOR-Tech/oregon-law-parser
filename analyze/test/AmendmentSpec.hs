...
spec = do
  ...
  describe "findChangedStatutes" $ do
    ...
    it "preserves four-digit UCC section suffixes in title evidence" $ do
      let title = "Relating to commercial transactions; amending ORS 71.1010 and 71.2010."
      findChangedStatutes title `shouldBe` ChangeSet { amended = ["71.1010", "71.2010"], repealed = [] }
  ...
  describe "findBodyChangedStatutes" $ do
    ...
    it "supports four-digit UCC section suffixes in operative clauses" $ do
      let ps = ["SECTION 1. ORS 71.1010 is amended to read:", "SECTION 2. ORS 71.2010 is amended to read:"]
      findBodyChangedStatutes ps `shouldBe` ChangeSet { amended = ["71.1010", "71.2010"], repealed = [] }
    ...
    it "chooses the earliest operative marker when a section has multiple repeal subclauses" $ do
      let ps = ["SECTION 1. (1) ORS 455.612, 455.614, 476.390, 476.394, 477.027, 477.161 and 477.490 and sections 12a, 12b and 29, chapter 592, Oregon Laws 2021, are repealed. (2) Section 12d, chapter 592, Oregon Laws 2021, as amended by section 10, chapter 611, Oregon Laws 2023, is repealed."]
      findBodyChangedStatutes ps `shouldBe` ChangeSet { amended = [], repealed = ["455.612", "455.614", "476.390", "476.394", "477.027", "477.161", "477.490"] }
