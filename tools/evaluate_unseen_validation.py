...
def main():
    ...
    for item in manifest["documents"]:
        ...
        if not result_path.exists():
            failures.append(f'{item["id"]}: missing parser result')
            fn += len(expected_pairs)
            documents.append({"id": item["id"], "parserFailed": True, "truePositives": 0,
                              "falsePositives": [], "falseNegatives": sorted([list(x) for x in expected_pairs]),
        ...
        if parser_errors:
            ...
            fn += len(expected_pairs)
            documents.append({"id": item["id"], "parserFailed": True, "parserErrors": parser_errors,
                              "truePositives": 0, "falsePositives": [],
                              "falseNegatives": sorted([list(x) for x in expected_pairs]), "metadataExactMatch": False})
            continue

        actual_pairs = section_pairs(actual.get("affectedSections", {}))
        doc_tp = len(expected_pairs & actual_pairs)
        doc_fp_set = actual_pairs - expected_pairs
        doc_fn_set = expected_pairs - actual_pairs
        tp += doc_tp
        fp += len(doc_fp_set)
        fn += len(doc_fn_set)
        metadata_matches = (
        ...
        documents.append({
            ...
            "falsePositives": sorted([list(x) for x in doc_fp_set]),
            "falseNegatives": sorted([list(x) for x in doc_fn_set]),
